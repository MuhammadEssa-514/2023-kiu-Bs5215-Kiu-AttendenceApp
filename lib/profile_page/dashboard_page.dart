            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                return GridView.count(
                    ...
                    children: [
                        HoverCard(icon: Icons.check_circle, title: "Take Attendance", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AttendancePage()))),
                        HoverCard(icon: Icons.people, title: "Manage Students", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ManageStudentsPage()))),
                        HoverCard(icon: Icons.analytics, title: "View Reports", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AttendanceReportPage(role: widget.role)))),
                        HoverCard(icon: Icons.calendar_month, title: "Schedule", onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SchedulePage()))),
                    ],
                );
              }
            ),
    ...
    if (image != null) {
      ...
      try {
          ...
          if (kIsWeb) {
             ...
             setState(() => _profileImageUrl = image.path); // Temporarily show local blob
          ...
          if (uploadedUrl != null) {
              ...
              await FirebaseFirestore.instance.collection('users').doc(user.uid).update({'photoUrl': uploadedUrl});
              ...
              if (widget.rollno != null) {
                  try {
                      ...
                      if (q.docs.isNotEmpty) {
                          ...
                          final q2 = await FirebaseFirestore.instance.collection('students').where('roll', isEqualTo: widget.rollno).get();
              ...
              if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Profile Picture Updated Everywhere!")));
          }

      } catch (e) {
          print("Upload Error: $e");
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Upload Error: $e"), backgroundColor: Colors.orange));
      } finally {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Student View
              if (widget.rollno != null && !isTeacher) 
  ...
  Widget _buildWelcomeCard(bool isTeacher) {
      return Container(
          ...
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                  Text("Welcome back,", style: const TextStyle(color: Colors.white70, fontSize: 16)),
                  ...
                  Container(
                      ...
                      child: Text(isTeacher ? "Instructor" : "Student${widget.rollno != null ? ' | ${widget.rollno}' : ''}", style: const TextStyle(color: Colors.white, fontSize: 12))
                  )
              ],
          ),
      );
        ...
        builder: (context, snapshot) {
          ...
          Widget buildUI(Map<String, dynamic> data) {
              ...
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                   const Text("My Courses & Reports", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                        ...
                        itemBuilder: (context, index) {
                           return Container(
                             ...
                             child: ListTile(
                               ...
                               title: Text(courses[index], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                               subtitle: Text("Section $studentSection", style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                               trailing: Container(
                                   padding: const EdgeInsets.all(6),
                                   decoration: BoxDecoration(color: Colors.blueGrey[50], shape: BoxShape.circle),
                                   child: const Icon(Icons.arrow_forward, size: 18, color: Colors.blueGrey)
                               ),
                               onTap: () {
                                  Navigator.push(
                                    context,
            ...
            builder: (context, localSnapshot) {
               ...
               if (jsonUse == null) {
                   for (String k in prefs.getKeys()) {
                       if (k.startsWith('student_') && k.substring(8).trim().toLowerCase() == targetRoll) {
                      ...
                      return Column(
                        children: [
                          Container(
                            ...
                            child: const Row(children: [Icon(Icons.wifi_off, color: Colors.orange), SizedBox(width:8), Text("Offline Mode")]),
                          ),
                          const SizedBox(height: 10),
                          buildUI(localData),
                        ],
               ...
               return Center(
                   child: Column(
                       children: [
                           ...
                           const Text("Student Record Not Found", style: TextStyle(color: Colors.grey))
                       ],
                   ),
               );
            }
                     ...
                     child: Column(
                       ...
                       children: [
                           ...
                           GestureDetector(
                               onTap: () async {
                                   await _pickImage();
                                   setStateDialog((){}); // Refresh dialog to show new image
                               },
                               child: Stack(
                                   children: [
                                       CircleAvatar(
                                         ...
                                         child: _isUploading
                                            ...
                                            : (_getAvatarImage() == null ? Text(_currentName.isNotEmpty ? _currentName[0].toUpperCase() : "U", style: const TextStyle(fontSize: 30, color: Colors.white)) : null),
                                       ),
                                       if (!_isUploading)
                                       Positioned(
                                           bottom: 0, right: 0,
                                           child: Container(
                                               ...
                                               child: const Icon(Icons.camera_alt, size: 16, color: Colors.blueGrey),
                                           ),
                                       )
                                   ],
                               ),
                           ...
                           const SizedBox(height: 10),
                           const Text("Tap to change picture", style: TextStyle(fontSize: 10, color: Colors.grey)),
                           const SizedBox(height: 20),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: _isHovered ? Colors.blueGrey.withOpacity(0.3) : Colors.grey.withOpacity(0.1),
